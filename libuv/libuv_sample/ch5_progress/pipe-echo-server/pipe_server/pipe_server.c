...
uv_loop_t *loop;
#define PIPE_PATH       "/tmp/zizy.server.sock"
...
static void _register(uv_stream_list_t *head, cmd_msg_t msg) 
{
    
}

static void _target(uv_stream_list_t *head, cmd_msg_t msg) 
{

}

static void _broadcast(uv_stream_list_t *head, cmd_msg_t msg) 
{
...
static void _listen(uv_stream_list_t *head, cmd_msg_t msg) 
{

}

typedef void (*_cmd_cb) (uv_stream_list_t *, cmd_msg_t);
static _cmd_cb _parse_cmd_cbs[] = {
    NULL,
    _register,
    _broadcast,
    _target,
...
void echo_read(uv_stream_t *client, ssize_t nread, const uv_buf_t *buf) {
    if (nread > 0) {
        #if 1
        buf->base[nread] = '\0';
        cmd_msg_t msg = parse_cmd(buf->base, nread);
        if(msg.cmd > 0 && msg.cmd < (sizeof(_parse_cmd_cbs) / sizeof(_parse_cmd_cbs[0]))) {
            _cmd_cb ptr = _parse_cmd_cbs[msg.cmd];
            if(ptr) {
                ptr(&head, msg);
            }
        }
