...
static st_villa_data villa_data;
...
int villa_parse(void *arg)
{
    ...
    char buf[1024] = "";
    lua_call_fun("/tmp/lua/villa_parse.lua", "lua_c_parse", "s>s", villa_data.data,buf);
    // lua_call_func("/tmp/lua/villa_parse.lua", "lua_c_parse", villa_data.data, villa_data.length, buf);
...
int villa_send(int fd, char *str, uint16_t cmd)
{
    if(fd < 0) {
        return -1;
    }
    memset(&villa_data, 0, sizeof(villa_data));
    villa_data.guid = 0xaa;
    villa_data.length = htons(strlen(str));
    villa_data.cmd = htons(cmd);
    strcpy((char *)villa_data.data, str);
    int i = 0;
    int len = strlen(str) + 4;
    uint8_t *ptr = (uint8_t *)&villa_data;
    ++ptr;
    for(i = 0; i < len; i++) {
        villa_data.chk ^= *ptr++;
    }
    villa_data.data[strlen(str)] = villa_data.chk;
    int ret = send(fd, (char *)&villa_data, len + 2, 0);
    if(ret != len + 2) {
        return -1;
    }
    return 0;
