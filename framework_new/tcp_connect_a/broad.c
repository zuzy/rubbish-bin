...
int main(int argc, char*argv[])
{
	...
	bro.sin_port = htons(PORT);
	bro.sin_addr.s_addr = htonl(INADDR_BROADCAST);
	...
	while (1)
	{
		...
		default:
			if (FD_ISSET(sock,&readfd))
			{
                ...
                sendto(sock, jsonbuf, strlen(jsonbuf), 0, (struct sockaddr *)&from_addr, from_len);
                printf("recv : %d %s\n", count, buffer);
				//from_addr保存客户端的地址结构
				if (strstr(buffer, IP_FOUND))
				{
					...
					count = sendto(sock, buffer, strlen(buffer), 0,
							(struct sockaddr*) &server_addr, from_len);
				}
			}
			break;
		}
