...
static int _manag_unit(uint8_t s_env_status, 
                int status_change, 
                int64_t now, 
                st_halo *handle,
                st_msg *m_ptr)
{
    ...
    list_for_each_entry_safe(ptr, tmp, handle->protos, list) {
        ...
        if(config <= s_env_status && ptr->active == 0) {
        ...
        if(status_change){
            ...
            if(config > s_env_status && ptr->active == 1) {
                print_main("close !");
                close_proto(ptr);
            }
        }
...
static int _manager(void *arg)
{
    ...
    uint8_t s_env_status = 0;
    int status_change = 0;
    int64_t now = get_now_s();
    // print_main("now %ld", now);
    #if 0

    #else
    if(wifi_connected) {
        s_env_status |= LOOP_CFG_WIFI;
    }
    if(alink_connect) {
        s_env_status |= LOOP_CFG_ALINK;
    }
    #endif
    if(s_env_status != handle->env_status) {
        handle->env_status = s_env_status;
        status_change = 1;
    ...
    if(list_empty(handle->msgs)) {
        _manag_unit(s_env_status, status_change, now, handle, NULL);
    } else {
        list_for_each_entry_safe(m_ptr, m_tmp, handle->msgs, list) {
            print_main("msg : %s", m_ptr->target);
            _manag_unit(s_env_status, status_change, now, handle, m_ptr);
            del_msg(m_ptr);
        }
    ...
    list_for_each_entry_safe(ptr, tmp, handle->protos, list) {
        // print_main("manage %s:%d",ptr->name, ptr->fd);
        config = ptr->config & 0x03;
        if(config <= s_env_status && ptr->active == 0) {
            if(ptr->callbacks->init_cb != NULL && ptr->callbacks->init_cb(ptr) == 0) {
                print_main("init %s success", ptr->name);
...
int update_status(st_hal hal)
{
    ...
    list_for_each_entry_safe(ptr, tmp, halo_handle->protos, list) {
        if(ptr->callbacks->active_send_cb != NULL) {
            memcpy(&ptr->hal_cmd, &hal, sizeof(hal));
...
int main(int argc, char *argv[])
{
    ...
    cJSON *body = cJSON_GetArrayItem(data, 0);
    if(body == NULL) {
        printf("body is null\n");
        return 0;
    }
    char *tmp = cJSON_Print(body);
    printf("temp is %s\n", tmp);
    free(tmp);

    cJSON *ids = cJSON_GetObjectItem(body, "ids");
    if(ids == NULL) {
        printf("ids is null\n");
        return 0;
    }
    if(ids->type == cJSON_Array) {
        int len = cJSON_GetArraySize(ids);
        int i = 0;
        for(; i < len; i++) {
            cJSON *num = cJSON_GetArrayItem(ids, i);
            switch(num->type) {
                case cJSON_Number: {
                    printf("get cjson num %d\n", num->valueint);
                    break;
                }
                case cJSON_String: {
                    printf("get cjson string %s\n", num->valuestring);
                    break;
                }
                default: break;
            }
    ...
    cJSON *tmp2 = cJSON_Parse(str);
    cJSON *tmp_data = cJSON_GetObjectItem(tmp2, "data");
    cJSON *tmp_pay = cJSON_GetArrayItem(tmp_data, 0);
    cJSON_DeleteItemFromObject(tmp_pay, "ids");
    char *out = cJSON_Print(tmp2);
    printf("out is %s\n", out);
    free(out);
    cJSON_Delete(tmp2);
