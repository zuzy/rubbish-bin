...
int tcp_ser_init(void *arg)
{
    ...
    socklen_t socklen;
    getsockname(pro->fd, (struct sockaddr*)&sockname, &socklen);
    print_common("get sockname %s %d\n", inet_ntoa(sockname.sin_addr), ntohs(sockname.sin_port));
    tcp_ser_handle.port = ntohs(sockname.sin_port);

    

    // set_nonblocking(pro->fd);
    pro->active = 1;
    print_ser("init tcp ser ok! %d", pro->fd);
    return 0;
...
int tcp_ser_accept(void *arg)
{
    ...
    if(fd >= 0) {
        print_ser("get a connection %d", fd);
