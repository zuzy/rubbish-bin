...
#define print_web(format, arg...)   do { printf("\033[31m[web]\033[0m:%s,%d--- "format"\n",__FILE__,__LINE__, ## arg);} while (0)
  ...
  if (ptr == NULL) {
   ...
   for (i = 0; extensions[i].ext != NULL; i++) {
    if (strcmp(s + 1, extensions[i].ext) == 0) {
     ...
     print_web("Opening \"%s\"\n", resource);
     if (fd1 == -1) {
      print_web("404 File not found Error\n");
      ...
      print_web("200 OK, Content-Type: %s\n\n",
      ...
      if (ptr == request + 4) // if it is a GET request
       ...
       if ((length = get_file_size(fd1)) == -1)
        print_web("Error in getting size !\n");
    ...
    if (i == size - 2) {
     print_web("415 Unsupported Media Type\n");
